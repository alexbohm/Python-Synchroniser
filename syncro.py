...
class single(object):
	...
	def active(self):
		if self.state==True:
			self.img = PhotoImage(file="%s/icons/button_on_highlight.gif" % (cwd))
			self.but.configure(image=self.img)
		else:
			self.img = PhotoImage(file="%s/icons/button_off_highlight.gif" % (cwd))
			self.but.configure(image=self.img)
	def deactive(self):
		if self.state==True:
			self.img = PhotoImage(file="%s/icons/button_on.gif" % (cwd))
...
class row(object):
	...
	def next(self):
		if self.act:
			self.buts[self.pos].deactive()
			self.pos += 1
			if self.pos > self.length+4:
				self.pos=5
			if self.buts[self.pos].state==True:
				self.snd.play()
			self.buts[self.pos].active()
	def destroy(self):
		self.act = False
		self.des = True
...
class master(object):
	...
	def start(self):
		...
		for row in self.rows:
			self.rows[row].buts[self.rows[row].pos].deactive()
			self.rows[row].pos = 5
		self.st = False
		self.loop()
	def loop(self):
		for row in self.rows:
			self.rows[row].next()
		if not self.st:
			win.after(self.delay, lambda: self.loop())
